    ...
    for (int i = static_cast<int>(m_document->layerCount()) - 1; i >= 0; --i)
    {
        ...
        connect(eyeBtn, &QPushButton::clicked, this,
                [this, i]()
                {
                    try
                    {
                        const auto& doc = myApp_->document();
                        auto layer = doc.layerAt(i);
                        if (!layer)
                            return;
                        myApp_->setLayerVisible(i, !layer->visible());
                    }
        ...
        connect(lockBtn, &QPushButton::clicked, this,
                [this, i, lockBtn, nameEdit, opacitySlider, item, isBottom]()
                {
                    try
                    {
                        const auto& doc = myApp_->document();
                        ...
                        if (!layer)
                            return;
                        myApp_->setLayerLocked(i, !layer->locked());
                        lockBtn->setIcon(
        ...
        connect(opacitySlider, &QSlider::sliderPressed, this,
                [this, opacitySlider]()
                {
                    const int idx = opacitySlider->property("layerIdx").toInt();
                    const auto& doc = myApp_->document();
                    auto layer = doc.layerAt(idx);
                    if (!layer)
                        return;
                    opacitySlider->setProperty("beforeOpacity",
                                               QVariant::fromValue(layer->opacity()));
                });

        connect(opacitySlider, &QSlider::valueChanged, this,
                [this, opacitySlider](int v)
                {
                    const int idx = opacitySlider->property("layerIdx").toInt();
                    myApp_->previewLayerOpacity(idx, static_cast<float>(v) / 100.f);
                });

        connect(opacitySlider, &QSlider::sliderReleased, this,
                [this, opacitySlider]()
                {
                    const int idx = opacitySlider->property("layerIdx").toInt();
                    const float before = opacitySlider->property("beforeOpacity").toFloat();
                    const float after = static_cast<float>(opacitySlider->value()) / 100.f;
                    try
                    {
                        myApp_->commitLayerOpacity(idx, before, after);
                    }
                    catch (const std::exception& e)
                    {
                        QMessageBox::critical(this, tr("Erreur"), e.what());
                    }
                });
...
void MainWindow::onLayerDoubleClicked(QListWidgetItem* item)
{
    ...
    if (auto nameEdit = row->findChild<QTextEdit*>())
    {
        ...
        nameEdit->setProperty("beforeName", nameEdit->toPlainText());
...
void MainWindow::onShowLayerContextMenu(const QPoint& pos)
{
    ...
    else if (act == resizeAct)
    {
        ...
        try
        {
            myApp_->resizeLayer(idx, app::Size{w, h});
        }
        catch (const std::exception& e)
        {
            QMessageBox::critical(this, tr("Error"), e.what());
    ...
    else if (act == deleteAct)
    {
        ...
        {
            myApp_->removeLayer(idx);
            populateLayersList();
        }
    }
    else if (act == mergeDownAct)
    {
        myApp_->mergeLayerDown(idx);
        populateLayersList();
    }
}
...
bool MainWindow::eventFilter(QObject* watched, QEvent* event)
{
    if (auto te = qobject_cast<QTextEdit*>(watched))
    {
        ...
        else if (event->type() == QEvent::FocusOut)
        {
            ...
            te->setReadOnly(true);
            const int idx = te->property("layerIdx").toInt();
            const QString before = te->property("beforeName").toString();
            const QString after = te->toPlainText();

            if (before != after)
            {
                try
                {
                    myApp_->renameLayer(static_cast<std::size_t>(idx), after.toStdString());
                    te->setProperty("beforeName", after);
                }
                catch (const std::exception& e)
                {
                    QMessageBox::critical(this, tr("Erreur"), e.what());
                    te->setPlainText(before);  // revert UI
                }
            }
...
void MainWindow::bucketFillAt(const QPoint& viewportPos)
{
    ...
    const auto& doc = myApp_->document();
    if (imgX < 0 || imgX >= doc.width() || imgY < 0 || imgY >= doc.height())
        return;
    ...
    try
    {
        const bool changed = myApp_->bucketFillAt(common::Point(imgX, imgY), newColor);
        statusBar()->showMessage(changed ? tr("Remplissage effectué") : tr("Aucun pixel modifié"),
                                 1500);
    }
    catch (const std::exception& e)
    {
